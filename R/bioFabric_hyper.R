#' Create an Interactive Hyperbolic BioFabric Plot
#'
#' bioFabric produces a static plot.  This uses the output from bioFabric
#' to create an interactive d3.js visualization projected on a hyperbolic grid.
#' Since it is an htmlwidget, it will work in all R contexts including the 
#' console, RStudio, rmarkdown, and Shiny.
#' 
...
#'
#' @example inst/examples/igraph_hyper.R
#' @example inst/examples/miserables_hyper.R
#' 
#' @import htmlwidgets
#'
#' @export
bioFabric_hyper <- function(
  data, zoomMin = 0.5, zoomMax = 15, width = NULL, height = NULL
) {
 
  # need to allow weights on vertices so that we can "stack" mutations
  message("Reminder: vertex and edge weights need to be supported!") 
